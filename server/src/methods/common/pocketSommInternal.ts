...
export interface PocketSommInput {
  imageUrl?: string;
  text?: string;
  depth?: Depth;
...
/**
 * Internal pocket-somm pairings logic. Callable by the endpoint wrapper
 * or by a router (e.g. smartScan). Uses the ambient request context for auth,
 * streaming, and IP resolution by default.
 *
 * Takes a photo, text, or transcribed voice. Returns 3-4 editorial drink
 * recommendations with chiaroscuro card photos for each.
 *
 ...
 */
export async function pocketSommInternal(input: PocketSommInput): Promise<PocketSommOutput> {
  ...
  try {
    ...
    await defaultStream({ status: imageUrl ? 'Reading the room...' : 'Considering...' });
...
Return ONLY a JSON object matching the example. No prose, no markdown fences.`;
    ...
    await defaultStream({ status: 'Pouring three glasses...' });
